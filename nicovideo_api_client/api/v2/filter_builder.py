...
from nicovideo_api_client.constants import FieldType, MatchValue


class FilterBuilder:
    def __init__(self, filter: Dict[str, Union[Dict[str, Union[str, int, datetime]], MatchValue]] = {}):
        self.filter: Dict[str, Union[Dict[str, Union[str, int, datetime]], MatchValue]] = filter
        ...
        if field_type is not None and literal1 is not None and value1 is not None:
            if literal2 is not None and value2 is not None:
                self.filter[field_type.value] = {
                    self.__cast_literal(literal1): value1,
                    self.__cast_literal(literal2): value2,
                }
            else:
    ...
    def match_filter(
        self, field_type: FieldType = None, value: Union[List[str], List[int], List[datetime]] = None
    ) -> "FilterBuilder":
        """
        一致検索用の辞書を組み立てる
